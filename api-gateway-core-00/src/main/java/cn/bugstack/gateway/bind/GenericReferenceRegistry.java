...
public class GenericReferenceRegistry {
    ...
    public void addGenericReference(String application, String interfaceName, String methodName){
        ...
        RegistryConfig referenceConfig = configuration.getRegistryConfig(application);
        ReferenceConfig<GenericService> reference = configuration.getReferenceConfig(interfaceName);
        // 构建Dubbo服务
        DubboBootstrap bootstrap = DubboBootstrap.getInstance();
        bootstrap.application(applicationConfig).registry(referenceConfig).reference(reference).start();
        // 获取泛化调用服务
